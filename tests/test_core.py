...
def test_pick_bucket_nearest_ar():
    ...
    assert pick_bucket(1.0).name == "sq"
    assert pick_bucket(4 / 3).name == "4x3"
    assert pick_bucket(16 / 9).name == "16x9"
    # near matches snap to the closest in log-AR space
    assert pick_bucket(1.05).name == "sq"
    assert pick_bucket(1.40).name == "4x3"
    assert pick_bucket(1.85).name == "16x9"
    # an ultrawide source still picks the widest available bucket
    assert pick_bucket(2.39).name == "16x9"
...
def test_pick_bucket_is_log_space():
    ...
    assert pick_bucket(mid * 1.001).name == "4x3"
    assert pick_bucket(mid * 0.999).name == "sq"


# ── compute_letterbox() ──────────────────────────────────────────────────────
def test_letterbox_exact_fit_no_pad():
    # a 960x540 source into the 16x9 canvas fits exactly, no scaling, no pad
    box = compute_letterbox(960, 540, 960, 540)
...
def test_letterbox_no_upscale_pads_small_source():
    ...
    box = compute_letterbox(480, 480, 960, 540)
...
def test_plan_encode_default_16x9_bucket():
    plan = plan_encode("in.mp4", "out.mp4", _probe(1920, 1080))
...
def test_plan_encode_fps_and_gop():
    ...
    plan2 = plan_encode("in.mp4", "out.mp4", _probe(1920, 1080), cfg)
    assert plan2.fps == 24
...
def test_plan_encode_fps_zero_keeps_source():
    ...
    plan = plan_encode("in.mp4", "out.mp4", _probe(1280, 720, fps=25.0), cfg)
    assert plan.fps == 25
    assert plan.gop == 25
...
def test_plan_encode_carries_crf_preset():
    ...
    plan = plan_encode("in.mp4", "out.mp4", _probe(1280, 720), cfg)
    assert plan.crf == 18
...
def test_build_ffmpeg_command_custom_bucket():
    ...
    plan = plan_encode("in.mp4", "out.mp4", _probe(1080, 1920), cfg)
