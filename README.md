...
Scaling uses **Lanczos resampling** (`flags=lanczos`): each output pixel is a weighted average of a neighborhood of source pixels via a windowed-sinc kernel.
Unlike a plain box/bilinear average, the kernel's negative side-lobes preserve edge sharpness and detail when shrinking (at the cost of slight ringing near hard edges).
...
The unit tests cover geometry (`even`, `pick_bucket`, `compute_letterbox`), planning (`plan_encode`), ffmpeg command construction, and path-list parsing (`read_path_list`) — and run **without ffmpeg installed**.
